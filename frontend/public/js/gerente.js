//Script para manipulação do Painel do Gerente
  ...
  import { getRelatorioReservasPorPeriodo, getRelatorioReservasPorMesa, getRelatorioReservasPorGarcom } from "./api.js";

  document.addEventListener("DOMContentLoaded", function () {
    // Exibe ou oculta os campos conforme o tipo de relatório selecionado
    document.getElementById("reportType").addEventListener("change", function () {
      const type = this.value;
      document.getElementById("commonFields").style.display = type === "periodo" ? "block" : "none";
      document.getElementById("garcomFields").style.display = type === "garçom" ? "block" : "none";
    ...
    document.getElementById("reportForm").addEventListener("submit", async function (e) {
      ...
      try {
        if (reportType === "periodo") {
          ...
          if (!garcomId) {
            showFeedback("Selecione um garçom para gerar o relatório.", "alert-danger");
            return;
          }
          ...
          if (!mesaId) {
            showFeedback("Selecione uma mesa para gerar o relatório.", "alert-danger");
            return;
          }
    ...
    function atualizarTabelaRelatorio(reservas) {
      ...
      if (reservas.length === 0) {
        showFeedback("Nenhum dado encontrado para os parâmetros informados.", "alert-warning");
        return;
      }
