...
import { atualizarStatusReserva, criarReserva, getReservas } from 'js/api.js'
...
document.addEventListener('DOMContentLoaded', async () => {
	...
	function handleError(error, defaultMessage) {
		...
		switch (status) {
			case 400:
				switch (message) {
					case 'Não foi possível buscar o garçom.':
						showFeedback('Não foi possível buscar o garçom.', 'alert-danger')
						break
					case 'Não foi possível buscar as mesas disponíveis.':
						showFeedback(
							'Não foi possível buscar mesas disponíveis.',
							'alert-danger',
						)
						break
					case 'Não foi possível buscar a reserva.':
						showFeedback('Não foi possível buscar a reserva.', 'alert-danger')
						break
					case 'Data inválida':
						showFeedback('Data inválida', 'alert-danger')
						break
					case 'Hora inválida':
						showFeedback('Hora inválida', 'alert-danger')
						break
					case 'Mesa inválida':
						showFeedback('Mesa inválida', 'alert-danger')
						break
					case 'Nome inválido':
						showFeedback('Nome inválido', 'alert-danger')
						break
					default:
						showFeedback(defaultMessage, 'alert-danger')
				}
				break
			case 404:
				if (error.message === 'Reserva inexistente') {
					showFeedback('Reserva inexistente', 'alert-danger')
				} else {
					showFeedback(defaultMessage, 'alert-danger')
				}
				break
			case 500:
				showFeedback('Erro interno do servidor.Tente novamente', 'alert-danger')
				break
			default:
				showFeedback(defaultMessage, 'alert-danger')
		}
